...
void PID::setup() {
  this->sensor_->add_on_state_callback([this](float state) {
    ...
    this->do_publish_ = roundf(state * 100) != roundf(this->current_value * 100);
    this->current_value = state;
    this->update_pid_();
  });
  this->current_value = this->sensor_->state;
  // restore set points
  ...
  if (restore.has_value()) {
    ...
    this->target_value = this->default_target_value_;
  }
}
...
void PID::update_pid_() {
  float value;
  if (std::isnan(this->current_value) || std::isnan(this->target_value)) {
    ...
    value = this->controller_.update(this->target_value, this->current_value);

    // Check autotuner
    if (this->autotuner_ != nullptr && !this->autotuner_->is_finished()) {
      auto res = this->autotuner_->update(this->target_value, this->current_value);
...
void PID::start_autotune(std::unique_ptr<PIDAutotuner> &&autotune) {
